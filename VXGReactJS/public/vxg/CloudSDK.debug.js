...
CloudHelpers.parseUriTests = function(){
	function test_check(url1,v1,v2,pn){
		if(v1 != v2){
			console.error("Expected value of " + pn + " '"  + v2 + "', but got '" + v1 + "' in url " + url1);
		}else{
			console.log("OK");
		}
	}
	var tests = [
		{
			url: "rtsp://aaa12:hjh@1111@100.224.0.117:1024/live/strm/tracks/601?starttime=20160616T191653Z",
			protocol: "rtsp",
			port: 1024,
			host: "100.224.0.117",
			user: "aaa12",
			...
			url: "rtsp://100.224.0.117/live/strm/tracks/601?starttime=20160616T191653Z",
			host: "100.224.0.117",
			protocol: "rtsp",
		},{
			...
			url: "http://10.20.16.128:8080?source=rtmp%3A//10.20.16.128%3A1935/live/u1m14c14_primary%3Fticket%3Dmedia.eyJuIjogInUxbTE0YzE0X3ByaW1hcnkifQ.1529657953.w-ziQmsqYdfmQ1kgmVdgaHCixrI",
			host: "10.20.16.128",
			port: 8080,
			path: "",
			protocol: "http",
	...
	for(var i in tests){
		var o = tests[i];
		var p = CloudHelpers.parseUri(o.url);
		test_check(o.url, p.protocol, o.protocol, "protocol");
		test_check(o.url, p.port, o.port, "port");
		test_check(o.url, p.host, o.host, "host");
		test_check(o.url, p.user, o.user, "user");
		test_check(o.url, p.path, o.path, "path");
		test_check(o.url, p.query, o.query, "query");
		test_check(o.url, p.password, o.password, "password");
	}
}
...
CloudHelpers.checkAutoplay = function(){
	var promise = document.createElement("video").play();
	var pr = window["Promise"];
	if(pr != undefined && promise instanceof Promise) {
		promise.catch(function(error) {
			...
			if(CloudHelpers.autoPlayAllowed) {
				console.log("checkAutoplay: Allowed")
			} else {
				console.log("checkAutoplay: Not allowed")
			}
		});
	} else {
			// Unknown if allowed
...
window.CloudPlayer = function(elid, options){
	...
	var mCallback_onError = null;
	var mCallback_onChannelStatus = null;
	...
	self.player.innerHTML = ''
		...
		+ ' muted=true autoplay=true preload playsinline="true"></video>'
	...
	self.vjs.on('error',function(error){
		...
		if(self.vjs.error() != null){
			...
			if(e.code == 4 && !CloudHelpers.supportFlash() && !CloudHelpers.isMobile()){
	...
	var el_error = self.player.getElementsByClassName('cloudplayer-error')[0];
	var el_error_text = self.player.getElementsByClassName('cloudplayer-error-text')[0];
	var el_player_time = self.player.getElementsByClassName('cloudplayer-time')[0];
	var el_big_play_button = self.player.getElementsByClassName('cloudplayer-big-play-button')[0];
	...
	var _sessionRecordsPos = 0;
	var _sessionRecords = null;
	var _sessionStartTime = null;
	var _current_session_vjs = null;
	var _next_session_vjs = null;
	
	function _executeCallbacks(evnt, args){
	...
	function _calculateTime(){
		if(mPosition != -1){
			return Math.floor(mCurrentPlayRecord.startUTC + self.vjs.currentTime()*1000);
		}
	...
	function _startPolingTime(){
		...
		mCurrentTimeInterval = setInterval(function(){
			if(mPlaying && !mStopped){
				var curr_time = 0;
				if (_source_type == 'session_records'){
					curr_time = Math.floor(_current_session_vjs.currentTime());
					curr_time += _sessionRecords[_sessionRecordsPos].startUTC - _sessionStartTime;
				} else if(_source_type == 'camera_records') {
					curr_time = mCurrentPlayRecord.startUTC + mCurrentRecord_vjs.currentTime()*1000;
				} else if(_source_type == 'camera_live') {
					// TODO webrtc
					if (mUsedPlayer == 'webrtc0' || mUsedPlayer == 'webrtc1' || mUsedPlayer == 'webrtc2') {
						...
						try{ _checkAndFixVideoSize(); }catch(e){ 
							// silent exception
						}
					}
				}else{
				...
				if(curr_time == self.time){
					...
					el_big_play_button.style.display = "none";
					
					if(_source_type == 'session_records'){
						self.time = curr_time;
						el_player_time.innerHTML = _formatTimeMS(self.time);
					}else if(_source_type == 'camera_records'){
						self.time = curr_time;
		...
		if(mCurrentPlayRecord != null){
			...
			mConn._getAPI().storageRecordsFirst(self.mSrc.getID(), start, 3).done(function(r){
				if(r.meta.total_count == 0){
					...
					for(var i = 0; i < len; i++){
						var _startNext = CloudHelpers.parseUTCTime(r.objects[i].start);
						if(_startNext >= _currEnd && mNextPlayRecord == null){
							mNextPlayRecord = r.objects[i];
							mNextPlayRecord.startUTC = CloudHelpers.parseUTCTime(mNextPlayRecord.start);
							mNextPlayRecord.endUTC = CloudHelpers.parseUTCTime(mNextPlayRecord.end);
		...
		mConn._getAPI().storageRecordsFirst(self.mSrc.getID(), start, nLimit).done(function(r){
			...
			for(var i = 0; i < len; i++){
				...
				if(firstRecordAfterT == null && rec.startUTC > pos){
					...
					console.log("firstRecordAfterT selected");
				}
			...
			if(!self.isRange() && mCurrentPlayRecord == null && nCountAfterT == 0){
				setTimeout(function(){
					...
					self.play();
					_executeCallbacks(CloudPlayerEvent.POSITION_JUMPED, {new_pos: CloudHelpers.getCurrentTimeUTC()});
				},10);
				return;
			}
			...
			if(!self.isRange() && mCurrentPlayRecord == null && firstRecordAfterT != null){
			...
			if(bSendEventPositionJumped){
				_executeCallbacks(CloudPlayerEvent.POSITION_JUMPED, {new_pos: pos});
			}

			mCurrentRecord_vjs = self.vjs;
			mNextRecord_vjs = self.vjs2;
	...
	function _polingSessionHLSList(sessionid){
		sessionid = sessionid || self.mSrc.getID();
		var xhr = new XMLHttpRequest();
		xhr.open('GET', self.mSrc.getLiveUrl_HLS());
		xhr.onload = function() {
			if(xhr.status === 200){
				if(self.mSrc.getID() != sessionid){
					return;
				}
				self._applyMediaTiket(self.mSrc.getLiveUrl_HLS(), self.mSrc.getLiveUrl_Expire());
				self._startPolingMediaTicket();
				
				self.vjs.src([{src: self.mSrc.getLiveUrl_HLS(),
				...
				if(self.mSrc.getID() != sessionid){
					console.warn("[CLOUDPLAYER] polingSessionHLSList, camid was changed stop poling hls list, currentCmaID=" + self.mSrc.getID() + ', camid=' + sessionid);
					return;
				}
				mTimeWaitStartStream++;
				if(mTimeWaitStartStream > _timeWaitStartStreamMax){
					self.stop("by _polingSessionHLSList");
					_showerror(CloudReturnCode.ERROR_STREAM_UNREACHABLE_HLS);
					return;
				}
				setTimeout(function(){
					console.warn("Wait one sec " + self.mSrc.getLiveUrl_HLS());
					xhr = null;
					_polingSessionHLSList(sessionid);
				},1000);
			}else{
	...
	function _loadSessionLiveUrl(){
		if(self.mSrc.type != 'session'){
			_showerror(CloudReturnCode.ERROR_INVALID_SOURCE);
			return;
		}

		self.mSrc.refresh().done(function(){
			_source_type = "session_live";
			if(!CloudHelpers.useHls()){
				self.vjs.ready(function(){
					console.log("Set url (rtmp): " + self.mSrc.getLiveUrl_Rtmp());
					self.vjs.options().flash.swf = CloudSDK.flashswf || 'last/video-js-by-vxg-buff200.swf';
					self.vjs.src([{src: self.mSrc.getLiveUrl_Rtmp(), type: 'rtmp/mp4'}]);
					_showBlackScreen();
				});
			}else{
				console.log("Set url (hls): " + self.mSrc.getLiveUrl_HLS());
				_startPolingTime();
				_polingSessionHLSList();

				/*self.vjs.src([{src: live_urls.hls,
					type: 'application/x-mpegURL'
				}]);*/
			}
	...
	function _preapreToPlayNextSessionRecord(){
		
		if(_sessionRecordsPos+1 < _sessionRecords.length){
			_next_session_vjs.src([{src: _sessionRecords[_sessionRecordsPos+1].url, type: 'video/mp4'}]);
			_next_session_vjs.one('loadeddata', function() {
				_next_session_vjs.pause();
			});
		}
		_current_session_vjs.one('ended', function() {
			_sessionRecordsPos++;
			if(_sessionRecordsPos < _sessionRecords.length){
				var t = _current_session_vjs;
				_current_session_vjs = _next_session_vjs;
				_next_session_vjs = t;
				_current_session_vjs.play();
				_next_session_vjs.el().style.display = "none";
				_current_session_vjs.el().style.display = "";
				console.log("Switched players");
				_preapreToPlayNextSessionRecord();
			}else{
				self.stop("by_preapreToPlayNextSessionRecord");
			}
		});
	}
	
	function _loadSessionRecords(_uniqPlay){
		if(self.mSrc.type != 'session'){
			_showerror(CloudReturnCode.ERROR_INVALID_SOURCE);
			return;
		}
		
		mConn._getAPI().getCamsessRecords(self.mSrc.getID()).done(function(r){
			// TODO redesign
			if(r.meta.total_count == 0){
				// TODO stop & show error
			}else{
				_sessionRecordsPos = 0;
				_sessionRecords = r.objects;
				_source_type = "session_records";
				for(var i in _sessionRecords){
					console.log(_sessionRecords[i]);
					_sessionRecords[i].startUTC = Math.floor(CloudHelpers.parseUTCTime(_sessionRecords[i].start)/1000);
					_sessionRecords[i].endUTC = Math.floor(CloudHelpers.parseUTCTime(_sessionRecords[i].end)/1000);
					if(i == 0){
						_sessionStartTime = _sessionRecords[i].startUTC;
						console.log(_sessionStartTime);
					}
				}
				
				_current_session_vjs = self.vjs;
				_next_session_vjs = self.vjs2;

				_current_session_vjs.src([{src: _sessionRecords[_sessionRecordsPos].url, type: 'video/mp4'}]);
				_preapreToPlayNextSessionRecord();
				
				self.vjs.one('loadeddata', function() {
					_hideloading();
					_initZoomControls();
					_initVolumeControls();
				});
				
				self.vjs2.one('loadeddata', function() {
					_hideloading();
					_initZoomControls();
					_initVolumeControls();
				});
	...
	function _loadLiveUrl(_uniqPlay){
		...
		}else if(self.mSrc.type == 'session'){
			if(!self.mSrc.isOnline()){
				_loadSessionRecords(); // _uniqPlay
			}else{
				_loadSessionLiveUrl(); // _uniqPlay
			}
		}else{
	...
	self.setSource = function(src){
		...
		_executeCallbacks(CloudPlayerEvent.SOURCE_CHANGED);
	}
	...
	function _vjs_play(){
		console.log("[PLAYER] vjs_play, mEvent: ", mEvent);
		// if(!mEvent && !CloudHelpers.autoPlayAllowed){
		var safari_and_hls = CloudHelpers.useHls() && CloudHelpers.isSafari();
		var is_mobile = CloudHelpers.isIOS() || CloudHelpers.isAndroid();
		var bFrameAndHLS = CloudHelpers.isFrame() && CloudHelpers.useHls();
		var bChromeAndHLS = CloudHelpers.isChrome() && !CloudHelpers.autoPlayAllowed && CloudHelpers.useHls();
		...
		if(!mEvent && (is_mobile || safari_and_hls || bFrameAndHLS || bChromeAndHLS)){
			_stopPolingTime();
			try{self.vjs.pause();}catch(e){console.warn("_vjs_play: skip error", e);}
			mTimeWaitStartStream = 0;
			// TODO show PlayButton
			console.warn('_vjs_play. is mobile. show big button');
			mShowedBigPlayButton = true;
			el_big_play_button.style.display = "block";
			el_big_play_button.onclick = function(event){
				mEvent = event;
				el_big_play_button.style.display = "none";
	...
	self.play = function(event){
		...
		if(self.mSrc.type != 'camera' && self.mSrc.type != 'session'){
			_showerror(CloudReturnCode.ERROR_INVALID_SOURCE);
			return;
		}
		...
		if(mPlaying){
			if(mPosition == -1){
				if ((mUsedPlayer == 'webrtc0' || mUsedPlayer == 'webrtc1' || mUsedPlayer == 'webrtc2')
					...
					return CloudHelpers.getCurrentTimeUTC() + (mConn ? mConn.getServerTimeDiff() : 0);
				} else if( (mUsedPlayer != 'webrtc0' && mUsedPlayer != 'webrtc1' && mUsedPlayer != 'webrtc2')
	...
	self.stop = function(who_call_stop){
		...
		if(mWebRTC1_Player != null){
			mWebRTC1_Player.stopWS();
			mWebRTC_el.style.display = 'none';
		};
	...
	function _initZoomControls(){
		...
		var el_videojs = document.getElementById(elid + '_vjs');
		var el_videojs2 = document.getElementById(elid + '_vjs2');
		var el_webrtc = self.player.getElementsByClassName('cloudplayer-webrtc')[0];
		...
		self.setNewZoom = function(v) {
			...
			if (self.currentZoom != v) {
				...
				el_videojs.style.transform = _scale_transform;
				el_videojs2.style.transform = _scale_transform;
				el_webrtc.style.transform = _scale_transform;
				...
				el_videojs2.style.left = '';
				el_videojs2.style.top = '';
				el_webrtc.style.left = '';
				el_webrtc.style.top = '';
		...
		self.zoomCursorMove = function(e){
			if(self.zoomCursorDownBool == true){
				...
				el_videojs2.style.left = left + '%';
				el_videojs2.style.top = top + '%';
				el_webrtc.style.left = left + '%';
				el_webrtc.style.top = top + '%';
			}
		}
	...
	function _initVolumeControls(){
		...
		self.mute = function(){
			...
			self.vjs2.volume(self.m.mute? 0: '' + self.m.volume.toFixed(1));
		}
	...
	function _polingLoadCameraLiveUrl_WebRTC1(_uniqPlay, live_urls){
		if (!live_urls.webrtc) {
			_showerror(CloudReturnCode.NOT_SUPPORTED_FORMAT);
			return;
		}
		mUsedPlayer = 'webrtc1';
		// WebRTC
		mElPlrType.innerHTML = "Used player: WebRTC (v1)";
		if(window.location.protocol.startsWith ("file")){
			self.player.showErrorText("Please open from browser");
			return;
		}
		...
		mNativeHLS_el.style.display = "none";
		mWebRTC_el.style.display = "block";
		if(!window['CloudPlayerWebRTC']){ // webrtc1
			console.error("Not found module CloudPlayerWebRTC");
			return;
		}

		var p = CloudHelpers.promise();

		if (CloudPlayerWebRTC.version != live_urls.webrtc.version) {
			console.warn("Expected version webrtc.version (v" + live_urls.webrtc.version + ") "
			+ " mismatch with included CloudPlayerWebRTC (v" + CloudPlayerWebRTC.version + ")");
			p = CloudHelpers.requestJS(live_urls.webrtc.scripts.player);
		} else {
			p.resolve();
		}

		p.done(function(){
			mWebRTC1_Player = new CloudPlayerWebRTC(mWebRTC_el, live_urls.webrtc.connection_url, live_urls.webrtc.ice_servers);
			mWebRTC1_Player.onServerError = function(event){
				console.error("[WebRTC1] Event error ", event);
				_showerror(CloudReturnCode.ERROR_WEBRTC_SERVER_ERROR);
				self.stop("by_webrtc1_error");
			}
			mWebRTC1_Player.startWS();
			_startPolingTime();
		})
	}
	...
	function _polingLoadCameraLiveUrl_WebRTC2(_uniqPlay, live_urls){
		...
		if (CloudPlayerWebRTC2.version != live_urls.webrtc.version) {
			console.warn("Expected version webrtc.version (v" + live_urls.webrtc.version + ") "
	...
	self._polingLoadCameraLiveUrl = function(_uniqPlay){
		...
		mConn._getAPI().cameraLiveUrls(self.mSrc.getID()).done(function(live_urls){
			...
			if (mPlayerFormatForced !== null) {
				...
				} else if (live_urls.webrtc && webrtc_major_version === 1 && mPlayerFormatForced === 'webrtc') {
					_polingLoadCameraLiveUrl_WebRTC1(_uniqPlay, live_urls);
			...
			if(self.mPlayerFormat == 'webrtc'){
				if(live_urls.rtc && CloudHelpers.supportWebRTC()){
					_polingLoadCameraLiveUrl_WebRTC0(_uniqPlay, live_urls);
				} else if (live_urls.webrtc && webrtc_major_version === 1 && CloudHelpers.supportWebRTC()){
					_polingLoadCameraLiveUrl_WebRTC1(_uniqPlay, live_urls);
	...
	self._reset_vjs = function(){
		...
		self.vjs2.muted(false);
		self.vjs2.autoplay(true);
		self.vjs2.volume(0.5);
		self.vjs2.el().style.display = "none";
	...
	self.initHLSMechanism = function(){
		...
		self._startPolingMediaTicket = function(_uniqPlay){
			self._stopPolingMediaTicket();
			console.warn("_startPolingMediaTicket");
			
			if(_uniqPlay != mUniqPlay) {
				console.warn("_uniqPlay not current [_startPolingMediaTicket]");
				return;
			}
			
			if(!videojs.Hls){
				console.error("Not found hls plugin");
				_showerror(CloudReturnCode.ERROR_NOT_FOUND_HLS_PLUGIN);
				return;
			}

			// set before request
			// console.warn("Set before request");
			videojs.Hls.xhr.beforeRequest = function(hls_options){
				// console.log("beforeRequest " + self.elid + " : ", hls_options);
				hls_options.withCredentials = false;

				// self.elid
				/*if(hls_options.uri.indexOf(".ts") + 3 == hls_options.uri.length){
					// hls_options.uri += self.hls_mediaticket_value;
					// hls_options.uri = hls_options.uri;
					// console.log("media-tiсket: m3u8 vxgcloudplayer.hls_mediaticket_value = " + hls_options.uri);
					return hls_options;
				}
				if(hls_options.uri.indexOf(".m3u8?") != -1){
					var new_uri = hls_options.uri.split("?")[0];
					if(new_uri.indexOf(".m3u8") + 5 == new_uri.length){
						// console.log("media-tiсket: m3u8 vxgcloudplayer.hls_mediaticket_value = " + self.hls_mediaticket_value);
						//hls_options.uri = new_uri + self.hls_mediaticket_value;
						// hls_options.uri = hls_options.uri;
						hls_options.withCredentials = false;
						console.log("beforeRequest 2: ", hls_options);
						return hls_options;
					}
				}*/
				return hls_options;
			...
			self._polingMediaTicketInterval = setInterval(function(){
				if(!mPlaying){
					self._stopPolingMediaTicket();
					return;
				}
				if(_uniqPlay != mUniqPlay) {
					console.warn("_polingMediaTicketInterval [_uniqPlay]");
					return;
				}

				var cur_time_utc = new Date().getTime() + mConn.getServerTimeDiff();
				var expire_after = mHLSLinkExpire - cur_time_utc;
				expire_after = Math.round(expire_after/1000);
				// console.log("media-tiсket for camid " + camid + ', expired after ' + expired_after + ' s');
				if(expire_after < 30){ // less then 30 sec
					console.log("media-tiсket: Updating");
					if(self.mSrc.type == "camera"){
						mConn._getAPI().cameraLiveUrls(self.mSrc.getID()).done(function(live_urls){
							if(_uniqPlay != mUniqPlay) {
								console.warn("_uniqPlay not current [cameraLiveUrls done]");
								return;
							}
							self._applyMediaTiket(live_urls.hls, live_urls.expire);
		...
		/*self._polingHLSList = function(live_urls, camid){
			var xhr = new XMLHttpRequest();
			xhr.open('GET', live_urls.hls);
			xhr.onload = function() {
				if(xhr.status === 200){
					if(self.currentCamID != camid){
						return;
					}
					self._applyMediaTiket(live_urls.hls, live_urls.expire);
					self._startPolingMediaTicket();
					if(CloudHelpers.containsPageParam("experimental_hls")){
						console.warn("Start experimental hls");
						// vxgcloudplayer.implHLSPlayer(self, live_urls.hls, live_urls.expire);
					}else{
						self.vjs.src([{src: live_urls.hls,
							type: 'application/x-mpegURL'
						}]);
					...
					if(self.currentCamID != camid){
						console.warn("[VXGCLOUDPLAYER] polingHLSList, camid was changed stop poling hls list, currentCmaID=" + self.currentCamID + ', camid=' + camid);
						return;
					}
					mTimeWaitStartStream++;
					if(mTimeWaitStartStream > _timeWaitStartStreamMax){
					...
					setTimeout(function(){
						console.warn("Wait one sec " + live_urls.hls);
						xhr = null;
						self._polingHLSList(live_urls, camid);
					},1000);
				}else{
					console.error("Unhandled");
				}
...
window.CloudPlayerWebRTC0 = function(videoEl, srv, rtmpUrl){
	...
	if (CloudHelpers.isSafari() ) {
        navigator.mediaDevices.getUserMedia({ "audio": false, "video": true}).then(function (stream) {
            console.log("[WEBRTC0] Camera permission granted");
	...
	self.handleIncomingError = function(error) {
		console.error("[WEBRTC0] IncomingError: ", error);
		resetState();
	}
	...
	self.onIncomingSDP = function(sdp) {
		...
		console.log("[WEBRTC0] Incoming SDP is " + JSON.stringify(sdp));
		peer_connection.setRemoteDescription(sdp).then(function(){
			console.log("Remote SDP set");
			if (sdp.type != "offer")
				return;
			console.log("[WEBRTC0] Got SDP offer, creating answer");
			peer_connection.createAnswer().then(self.onLocalDescription).catch(function(t){
				console.error('[WEBRTC0] createAnswer: ', t);
			});
		}).catch(function(t){
			console.error('[WEBRTC0] setRemoteDescription: ', t);
		});
	}

	// Local description was set, send it to peer
	self.onLocalDescription = function(desc) {
		console.log("[WEBRTC0] Got local description: " + JSON.stringify(desc));
		peer_connection.setLocalDescription(desc).then(function() {
			console.log("[WEBRTC0] Sending SDP answer");
			sdp = {'sdp': peer_connection.localDescription}
			ws_conn.send(JSON.stringify(sdp));
			console.warn("[WEBRTC0] Streaming");
		});
	}

	// ICE candidate received from peer, add it to the peer connection
	self.onIncomingICE = function(ice) {
		console.log("[WEBRTC0] Incoming ICE: " + JSON.stringify(ice));
		var candidate = new RTCIceCandidate(ice);
		peer_connection.addIceCandidate(candidate).catch(function(t){
			console.error('[WEBRTC0] addIceCandidate ', t);
		});
	}

	self.onServerMessage = function(event) {
		console.log("[WEBRTC0] Received " + event.data);
		switch (event.data) {
			case "HELLO":
				console.log("[WEBRTC0] Registered with server, waiting for stream");
				return;
			default:
	...
	self.startWS = function() {
		...
		ws_conn.addEventListener('open', function(event) {
			...
			console.log("[WEBRTC0] Registering with server");
...
window.CloudPlayerWebRTC = function(objVideoEl, strConnectionUrl, arrIceServers){
	// for Cloud
	...
	if (CloudHelpers.isSafari() ) {
        navigator.mediaDevices.getUserMedia({ "audio": false, "video": true}).then(function (stream) {
            console.log("[WEBRTC] Camera permission granted");
        }).catch(function(a1, a2){
			console.error(a1, a2)
		});
    }
	...
	self.onIncomingSDP = function(sdp) {
		sdp.sdp = sdp.sdp.replace(/profile-level-id=[^;]+/, 'profile-level-id=42e01f');
		console.log("[WEBRTC] Incoming SDP is "+ JSON.stringify(sdp));
		peer_connection.setRemoteDescription(sdp).then(function(){
			console.log("Remote SDP set");
			if (sdp.type != "offer")
				return;
			console.log("[WEBRTC] Got SDP offer, creating answer");
			peer_connection.createAnswer().then(self.onLocalDescription).catch(function(t){
				console.error('[WEBRTC] ' + t);
			});
		}).catch(function(t){
			console.error('[WEBRTC] ' + t);
		});
	...
	// Local description was set, send it to peer
	self.onLocalDescription = function(desc) {
		console.log("[WEBRTC] Got local description: " + JSON.stringify(desc));
		peer_connection.setLocalDescription(desc).then(function() {
			console.log("[WEBRTC] Sending SDP answer");
			sdp = {'sdp': peer_connection.localDescription}
			ws_conn.send(JSON.stringify(sdp));
			console.error("[WEBRTC] Streaming");
		});
	}

	// ICE candidate received from peer, add it to the peer connection
	self.onIncomingICE = function(ice) {
		console.log("[WEBRTC] Incoming ICE: " + JSON.stringify(ice));
		var candidate = new RTCIceCandidate(ice);
		peer_connection.addIceCandidate(candidate).catch(function(t){
			console.error('[WEBRTC] ' + t);
		});
	...
	self.onServerMessage = function(event) {
		...
		switch (event.data) {
			case "HELLO":
				console.log("[WEBRTC] Registered with server, waiting for stream");
    			return;
			default:
				if (event.data.startsWith("ERROR")) {
					self.handleIncomingError(event.data);
	...
	self.onServerClose = function(event) {
		console.error("Closed WebRTC "+event.reason)
		self.resetVideoElement();
	...
	self.onServerError = function(event) {
		console.error("[WEBRTC] Unable to connect to server, did you add an exception for the certificate?")
	}
	...
	self.onRemoteStreamAdded = function(event) {
		...
		if (videoTracks.length > 0) {
			console.log('[WEBRTC] Incoming stream: ' + videoTracks.length + ' video tracks and ' + audioTracks.length + ' audio tracks');
			m_objVideoEl.srcObject = event.stream;
		} else {
			self.handleIncomingError('[WEBRTC] Stream with unknown tracks added, resetting');
		}
	}
	...
	self.createCall = function(msg) {
		...
		peer_connection = new RTCPeerConnection(m_objRTCConfiguration);
		peer_connection.onaddstream = self.onRemoteStreamAdded;
		/* Send our video/audio to the other peer */

		if (!msg.sdp) {
			console.log("[WEBRTC] WARNING: First message wasn't an SDP message!?");
		}
		...
		// same uuid
		if (event.candidate == null) {
				console.warn("[WEBRTC] ICE Candidate was null, done");
				return;
		}
		ws_conn.send(JSON.stringify({'ice': event.candidate}));
		};

		console.log("[WEBRTC] Created peer connection for call, waiting for SDP");
	}
	
	self.startWS = function() {
		self.connect_attempts++;
		if (self.connect_attempts > 3) {
			console.error("[WEBRTC] Too many connection attempts, aborting. Refresh page to try again");
			return;
		}
		console.log("[WEBRTC] Connecting to server...");
		loc = null;
		
		ws_conn = new WebSocket(m_strConnectionUrl);

		/* When connected, immediately register with the server */
		ws_conn.addEventListener('open', function(event) {
			ws_conn.send('HELLO ' + window.CloudPlayerWebRTC.version);
			console.log("[WEBRTC] Registering with server");
			ws_conn.send('SPAWN')
		});
		ws_conn.addEventListener('error', self.onServerError);
		ws_conn.addEventListener('message', self.onServerMessage);
...
window.CloudPlayerWebRTC2 = function(objVideoEl, strConnectionUrl, arrIceServers, options) {
    ...
    self.createPublishingConnection = function(strSessionPartnerPeerUID) {
        ...
        navigator.mediaDevices.getUserMedia({audio: m_bSendAudio, video: m_bSendVideo}).then(function (objLocalStream) {
            ...
            objPeer.createOffer().then(function(offer) {
                ...
                console.error("[WEBRTC2] Streaming");
            }).catch(function(t){
                console.error('[WEBRTC2] ', t);
            });
        }).catch(function(a1, a2){
            console.error(a1, a2);
        });
	...
	self.onServerError = function(event) {
		console.error("[WEBRTC2] Unable to connect to server, did you add an exception for the certificate?")
	}
...
window.CloudPlayerWebRTC2.version = "2.0.0";
...
window.CloudCameraTimelineView = function(viewid, options){
	...
	self.setRange = function(startPos,endPos){
		...
		var end_t = mRangeMax;
		var len_mode_ms = mModes["HOUR_MODE"].len_ms;
		if(range_len <= len_mode_ms){
			self.hideArrowsButtons();
		}else{
			end_t = mRangeMin + len_mode_ms;

			// correct if current time
			var t = CloudHelpers.getCurrentTimeUTC();
			if(t < mRangeMax && t > mRangeMin){
				if(t > end_t){
					end_t = t + Math.floor(len_mode_ms/2);
					if(end_t > mRangeMax){
						end_t = mRangeMax;
					}
					start_t = end_t - len_mode_ms;
				}
			}
		}
		var newmode = clone(mModes["HOUR_MODE"]);
...
window.CloudCameraCalendarView = function(elem, options){
	...
	function _generateMonthDays(nYear, nMonth) {
		...
		var prevEndDay = new Date(prevYear, prevMonth, 0).getDate();
...
CloudSDK.version = '2.0.52';
CloudSDK.datebuild = '181102';
			...
			navigator.mediaDevices.getUserMedia({video: true}).then(function(stream) {
				...
				if (self.conn) {
					...
					self.conn._getAPI().cameraStreamUrls_webrtc(self.mCameraID).done(function (stream_urls) {
						...
						if (CloudPlayerWebRTC2.version != stream_urls.webrtc.version) {
